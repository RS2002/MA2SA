...
#         self.q_linear = nn.ModuleList()
#         self.k_linear = nn.ModuleList()
#         self.dim = input_dims // head
#         for i in range(int(head)):
#             self.q_linear.append(MLP([self.dim,hidden_dims,hidden_dims], dropout=dropout))
#             self.k_linear.append(MLP([self.dim,hidden_dims,hidden_dims], dropout=dropout))
#         self.head = head
#         self.method = method
#
#         if self.method != "mean":
...
#
#         for i in range(self.head):
#             query=self.q_linear[i](q[:,i*self.dim:(i+1)*self.dim])
#             key=self.k_linear[i](k[:,i*self.dim:(i+1)*self.dim])
#             attn = torch.mm(query,key.T)
#             if self.head == 1:
#                 return attn
#
...
class AC_BERT(nn.Module):
    def __init__(self, bertconfig_actor, bertconfig_critic, state_size=7, history_order_size=4, current_order_size=5, hidden_dim=64, agent_num=1000, bi_direction=False, dropout=0.0):
        ...
        # self.attention = QK_Attention(input_dims=hidden_dim, hidden_dims=hidden_dim, head=4, dropout=dropout)

...
class Img_Buffer():
    ...
    def append(self, img, episode=0, end_time=False):
        ...
        img = img.tolist()
        self.img.append(img)
        if (not end_time) and self.num>1:
            self.img_next[-1] = img
...
class Worker():
    def __init__(self, buffer, img_buffer, lr=0.0001, gamma=0.99, max_step=60, num=1000, device=None, zone_table_path = "./data/Manhattan_dic.pkl", model_path = None, njobs = 24, bi_direction = True, dropout = 0.0, compression = False, rand_init=False, restrict = False, noise_type=0):
        super().__init__()
        self.buffer = buffer
        ...
        self.Q_training = Assignment_Net(state_size=7, history_order_size=5, current_order_size=5, hidden_dim=64, bi_direction=bi_direction, dropout=dropout).to(device)
        self.Q_target = Assignment_Net(state_size=7, history_order_size=5, current_order_size=5, hidden_dim=64, bi_direction=bi_direction, dropout=dropout).to(device)
        ...
        self.Q_target.eval()
        print('Platform total parameters:', sum(p.numel() for p in self.Q_training.parameters() if p.requires_grad) + sum(p.numel() for p in self.ViT.parameters() if p.requires_grad))
    ...
    def update(self, feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, assignment_table, assignment_state, final_step=False, episode=1):
        ...
        # add img buffer
        if self.is_train:
            self.img_buffer.append(self.img, episode, final_step)
        # # update idle time

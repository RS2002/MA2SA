...
# class ViT(nn.Module):
#     def __init__(self, input_channel=3, image_size=112, patch_size=8, num_layers=3, num_heads=4, hidden_dim=64,
#                  mlp_dim=3072):
#         super().__init__()
#
#         if input_channel != 3:
#             self.conv = nn.Conv2d(input_channel, 3, kernel_size=1)
#         else:
#             self.conv = None
#
...
class ViT(nn.Module):
    def __init__(self, bertconfig, input_dim=11, hidden_dim=64):
        super().__init__()
    ...
    def encode(self,x):
        x = self.mlp(x)
        x_emb = self.vit(inputs_embeds=x, attention_mask=None, output_hidden_states=False)
        x_emb = x_emb.last_hidden_state
        return x_emb

    def forward(self,x):
        x_emb = self.encode(x)
        x_recover = self.recoverer(x_emb)
        x_next = self.predictor(x_emb)
        return x_recover, x_next

class AC_BERT(nn.Module):
    def __init__(self, vit_config, bertconfig_actor, bertconfig_critic, state_size=7, history_order_size=4, current_order_size=5, hidden_dim=64, agent_num=1000, bi_direction=False, dropout=0.0):
        super().__init__()

        # self.ViT = ViT(**vit_config)
        # self.ViT = MLP([img_size,hidden_dim,hidden_dim],arl=True)
        self.ViT = ViT(**vit_config)
    ...
    def encode(self, img, order, x_state, x_order, order_num=None):
        img_emb = self.ViT.encode(img.unsqueeze(0))

        worker, order = self.assignment_net.encode(order, x_state, x_order, order_num)
        x = torch.concat([worker, order, img_emb.squeeze(0)], dim=0).unsqueeze(0)
        # x = torch.concat([worker, order, img_emb], dim=0).unsqueeze(0)
        ...
        x_emb = x_emb[:,:-img_emb.shape[1],:]
        # x_emb = x_emb[:,:-img_emb.shape[0],:]
        return x_emb

    def act(self, img, order, x_state, x_order, order_num=None, output_prob=False):
        img = img.float()

        x_emb = self.encode(img, order, x_state, x_order, order_num)
    ...
    def forward(self,img,order,x_state,x_order,action,order_num=None):
        p_matrix, x_emb = self.act(img, order, x_state, x_order, order_num)
        valid_indices = (action != -1)

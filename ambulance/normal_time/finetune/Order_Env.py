...
class Demand():
    def __init__(self, demand_path):
        columns_to_keep = ['day', 'hour', 'minute', 'INITIAL_SEVERITY_LEVEL_CODE', 'lat', 'lng'] # INITIAL_SEVERITY_LEVEL_CODE 1 -> 8: dangerous -> tiny
        self.demand = pd.read_excel(demand_path, usecols=columns_to_keep)
    ...
    '''
    def reset(self, day = 1, hour = 0, start_time = 0, p_sample = 0.95, wait_time = 60, compression=False, max_step = 60):
        if compression:
            self.filtered_demand = self.demand[(self.demand["day"] == day) & (self.demand["hour"] <= 2 + hour) & (self.demand["hour"] >= hour) & (self.demand["minute"] < max_step)]
        else:
            self.filtered_demand = self.demand[(self.demand["day"] == day) & (self.demand["hour"] == hour) & (self.demand["minute"] < max_step)]

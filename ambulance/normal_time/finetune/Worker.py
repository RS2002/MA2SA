...
class Worker():
    def __init__(self, buffer, lr=0.0001, gamma=0.99, max_step=60, num=1000, device=None, hospital_table_path = "./data/hospital.xlsx", EMS_table = "./data/EMS.xlsx", model_path = None, njobs = 24, bi_direction = True, dropout = 0.0, compression = False, pretrain_model_path = None):
        super().__init__()
        self.buffer = buffer
        ...
        bertconfig_actor = BertConfig(max_position_embeddings=max_len, hidden_size=64, num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")
        bertconfig_critic = BertConfig(max_position_embeddings=400, hidden_size=128, num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")

        self.AC_training = AC_BERT(bertconfig_actor, bertconfig_critic, state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, agent_num=1000, bi_direction=bi_direction, dropout=dropout).to(device)
        self.AC_target = AC_BERT(bertconfig_actor, bertconfig_critic, state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, agent_num=1000, bi_direction=bi_direction, dropout=dropout).to(device)
    ...
    def train(self, batch_size=8, train_times=1, show_pbar=False, train_actor=False, train_critic=True):
        ...
        for _ in pbar:
            ...
            for i in range(len(worker_state)):
                ...
                if train_actor:
                    worker_state_temp, order_state_temp, order_num_temp, pool_order_temp = worker_state_temp.to(self.device), order_state_temp.to(self.device), order_num_temp.to(self.device), pool_order_temp.to(self.device)
                    x1, x2, x3 = norm(pool_order_temp, worker_state_temp, order_state_temp)

...
beta_list[1]: punishment of picking up time (user waiting time : hour)
beta_list[2]: punishment of delivery time
'''
def reward_func_generator(beta_list, threshold):
    def reward_func(time_add,call_type,pickup_time,direct_distance):
        call_type = 9 - call_type
        r = beta_list[0] - beta_list[1] * call_type * pickup_time / 60 - beta_list[2] * call_type/2 * time_add / 60
        return r
...
def execute(observe, current_order_state, current_order_num, assignment, new_orders_state, time_threshold, reward_func, current_time):
    if assignment is not None and observe[4] == 0:
        ...
        plat, plon, dlat, dlon, appear_time, call_type = new_orders_state[assignment]
        ...
        new_total_travel_time = new_total_travel_time + pickup_time + confirmation_time
        ...
        reward = reward_func(time_add,call_type,pickup_time+confirmation_time,direct_distance)

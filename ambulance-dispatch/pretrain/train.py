from Worker import Buffer, Worker, Img_Buffer
...
def get_args():
    ...
    parser.add_argument("--zone_table_path",type=str,default="../data/NY_dic.pkl")
    parser.add_argument("--demand_path",type=str,default="../data/Record.xlsx")
    parser.add_argument("--hospital_path",type=str,default="../data/hospital.xlsx")
    parser.add_argument("--ems_path",type=str,default="../data/EMS.xlsx")
...
def main():
    ...
    buffer_img = Img_Buffer(capacity=args.buffer_capacity, episode_capacity=args.buffer_episode)

    worker = Worker(buffer, buffer_img, lr=args.lr, gamma=args.gamma, max_step=args.max_step, num=args.worker_num, device=device, zone_table_path = args.zone_table_path, hospital_table_path = args.hospital_path, EMS_table = args.ems_path, model_path = args.model_path, njobs = args.njobs, bi_direction = args.bi_direction, dropout = args.dropout, compression = compression)
    ...
    while True:
        ...
        for t in pbar:
            ...
            worker.update(feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, assignment, order_state, (t==args.max_step - 1), j, t)
            ...
            if counter == 0 and buffer.num!=0:
                loss, loss_img = worker.train(args.batch_size, 1, False)
                loss_list.append(loss)
                loss_list_img.append(loss_img)

            counter = (counter + 1) % train_step
        # loss = worker.train(args.batch_size,args.train_times)
        loss = np.mean(loss_list)
        loss_img = np.mean(loss_list_img)
        worker.schedule.step()
        # worker.update_target()
        worker.schedule_vit.step()

        Pickup_Num = platform.Pickup_Num
        Finish_Num = len(worker.Pass_Travel_Time)
        ...
        log = f"Epoch {j} | Reward: {reward} , Loss: {loss} , Served Order: {Pickup_Num} , Delivery Time: {Delivery} , Finished Order: {Finish_Num} , Pickup Time: {Pickup} , Confirmation Time: {Confirmation} , Loss (ViT): {loss_img}"
        print(log)
        ...
        if j % args.eval_episode == 0 :
            ...
            log = f"Eval {j} | Reward: {reward}, Served Order: {Pickup_Num} , Delivery Time: {Delivery} , Finished Order: {Finish_Num} , Pickup Time: {Pickup} , Confirmation Time: {Confirmation}"
            print(log)

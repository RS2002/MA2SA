import numpy as np
import torch
from model import Assignment_Net, ViT
...
class Img_Buffer():
    def __init__(self, capacity=1000, episode_capacity=10):
        super().__init__()
    ...
    def append(self, img, episode=0, start_time=False, end_time=False):
        ...
        pad = np.zeros_like(img).tolist()
        img = img.tolist()
        self.img.append(img)
        if not start_time:
            self.img_next[-1] = img
        self.img_next.append(pad)
    ...
    def sample(self,size,device):
        ...
        img = torch.tensor([self.img[i] for i in indices]).to(device)
        img_next = torch.tensor([self.img_next[i] for i in indices]).to(device)
...
def mask(x, device, min_prob = 0.05, max_prob = 0.30):
    x = x.clone()
    batch_size, seq_len, feature_dim = x.shape
    rand_word = torch.randn((batch_size, seq_len, feature_dim)).to(device)
    loss_mask = torch.zeros([batch_size, seq_len]).to(device)
    chosen_num_min = int(seq_len * min_prob)
    chosen_num_max = int(seq_len * max_prob)
    ...
    loss_mask = loss_mask.unsqueeze(2).repeat(1, 1, feature_dim)
    loss_mask = 1 - loss_mask
    avg = torch.sum(x * loss_mask, dim=1, keepdim=True) / (torch.sum(loss_mask, dim=1, keepdim=True) + 1e-8)
    std = torch.sqrt(torch.sum(((x - avg) ** 2) * loss_mask, dim=1, keepdim=True) / (
            torch.sum(loss_mask, dim=1, keepdim=True) + 1e-8))
    rand_word = rand_word * std + avg
    x[loss_mask.bool()][2:] = rand_word[loss_mask.bool()][2:]
    return x, loss_mask
...
class Worker():
    def __init__(self, buffer, img_buffer, lr=0.0001, gamma=0.99, max_step=60, num=1000, device=None, zone_table_path = "./data/NY_dic.pkl", hospital_table_path = "./data/hospital.xlsx", EMS_table = "./data/EMS.xlsx", model_path = None, njobs = 24, bi_direction = True, dropout = 0.0, compression = False, rand_init=False, restrict = False, noise_type=0):
        ...
        self.Q_training = Assignment_Net(state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, bi_direction=bi_direction, dropout=dropout).to(device)
        self.Q_target = Assignment_Net(state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, bi_direction=bi_direction, dropout=dropout).to(device)
        ...
        self.ViT = ViT(bertconfig, input_dim=11, hidden_dim=64).to(self.device)
        self.ViT.train()
        ...
        self.optim_vit = torch.optim.Adam(self.ViT.parameters(), lr=lr)
        self.schedule_vit = torch.optim.lr_scheduler.ExponentialLR(self.optim_vit, gamma=0.99)

        self.njobs = njobs
        self.loss_func = nn.MSELoss()
        self.loss_func_full = nn.MSELoss(reduction="none")
    ...
    def save(self, path):
        ...
        torch.save(self.ViT.state_dict(), "ViT_"+path)
    ...
    def train(self, batch_size=256, train_times=1, show_pbar=False):
        ...
        # self.schedule.step()
        torch.set_grad_enabled(False)
        return np.mean(loss_list), self.train_vit()

    def train_vit(self, batch_size=32, train_times=1, show_pbar=False):
        torch.set_grad_enabled(True)
        self.ViT.train()
        ...
        loss_list = []
        for _ in pbar:
            img, img_next = self.img_buffer.sample(batch_size, self.device)
            loss_mask_next = (img_next.sum(dim=(1, 2)) != 0).float()

            img = img_norm(img)
            img_next = img_norm(img_next)

            x, loss_mask = mask(img,self.device)
            # print(x.shape)
            y_recover, y_next = self.ViT(x)

            loss_recover = self.loss_func_full(y_recover,img)
            loss_recover = (torch.mean(loss_recover) + torch.sum(loss_recover * loss_mask) / (torch.sum(loss_mask)+1e-8)) / 2
            loss_next = self.loss_func_full(y_next,img_next)
            loss_next = torch.sum(loss_next.mean(dim=(1,2))*loss_mask_next) /  (torch.sum(loss_mask_next)+1e-8)

            loss = (loss_recover + loss_next) / 2

            self.optim_vit.zero_grad()
            loss.backward()
            # self.scaler.scale(loss).backward()  # 缩放损失并反向传播
            torch.nn.utils.clip_grad_norm_(self.ViT.parameters(), 1.0)  # avoid gradient explosion
            has_nan = False
    ...
    def update(self, feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, assignment_table, assignment_state, final_step=False, episode=1, current_time=0):
        # update each worker state parallely
        results = Parallel(n_jobs=self.njobs)(
            delayed(single_update)(self.observe_space[i], self.current_orders[i], self.current_order_num[i], self.travel_route[i], self.travel_time[i], feedback_table[i], new_route_table[i], new_route_time_table[i], new_remaining_time_table[i], new_total_travel_time_table[i], new_detour_table[i], self.experience[i], self.go_back)
        ...
        # add img buffer
        if self.is_train:
            self.img_buffer.append(self.img, episode, (current_time==0) , final_step)
        # # update idle time
        # self.observe_space[:, 8] += 1
        # self.observe_space[self.current_order_num != 0, 8] = 0

def single_update(observe_space, current_orders, current_orders_num, current_travel_route, current_travel_time, feedback, new_route ,new_route_time, new_remaining_time, new_total_travel_time, new_detour_table, experience, go_back):

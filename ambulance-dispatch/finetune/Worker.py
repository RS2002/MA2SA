...
class Buffer():
    ...
    def append(self, experience, episode=0):
        if self.num > 0 and self.episode[0]<episode-self.episode_capacity:
            ...
            self.img = self.img[old_record_num:]
            self.img_next = self.img_next[old_record_num:]
    ...
    def sample(self,size):
        ...
        # indices = np.random.choice(self.num, size, replace=False)
        ...
        # reward = [(self.reward[i]-reward_mean)/reward_std for i in indices]

        return img, worker_state, order_state, order_num, pool_order, action, reward, img_next, worker_state_next, order_state_next, order_num_next, pool_order_next, action_next

def calculate_entropy(log_probs, probs):
    entropy = -torch.sum(probs * log_probs, dim=1)
    return torch.mean(entropy)

def norm(order_state, worker_state, history_order_state, lat_min = 40.50817, lat_max = 40.90002, lon_min = -74.24254, lon_max = -73.67979, simulation_time = 60, max_capacity = 3):
    ...
    worker_state[:, 6] = (worker_state[:, 6] - lat_min) / lat_range
...
class Worker():
    def __init__(self, buffer, lr=0.0001, gamma=0.99, max_step=60, num=1000, device=None, zone_table_path = "./data/NY_dic.pkl", hospital_table_path = "./data/hospital.xlsx", EMS_table = "./data/EMS.xlsx", model_path = None, njobs = 24, bi_direction = True, dropout = 0.0, compression = False, pretrain_model_path = None, rand_init = False, restrict = False, noise_type = 0):
        super().__init__()
        self.buffer = buffer
        ...
        self.hospital_tree = KDTree(self.hospital)
        ...
        max_len = 1000
        bertconfig_actor = BertConfig(max_position_embeddings=max_len, hidden_size=64, intermediate_size=256, num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")
        bertconfig_critic = BertConfig(max_position_embeddings=400, hidden_size=128, intermediate_size=512, num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")
        # bertconfig_actor = BertConfig(max_position_embeddings=max_len, hidden_size=64,
                                    #   num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")
        # bertconfig_critic = BertConfig(max_position_embeddings=400, hidden_size=128,
                                    #    num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")

        bertconfig =  BertConfig(max_position_embeddings=len(self.coordinate_lookup_lat), hidden_size=64, intermediate_size=256, num_hidden_layers=3, num_attention_heads=4, position_embedding_type="none")
        vit_config = {"bertconfig":bertconfig,"input_dim":11,"hidden_dim":64}

        self.AC_training = AC_BERT(vit_config, bertconfig_actor, bertconfig_critic, state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, agent_num=1000, bi_direction=bi_direction, dropout=dropout).to(device)
        self.AC_target = AC_BERT(vit_config, bertconfig_actor, bertconfig_critic, state_size=8, history_order_size=5, current_order_size=6, hidden_dim=64, agent_num=1000, bi_direction=bi_direction, dropout=dropout).to(device)

        if pretrain_model_path is not None:
            self.load_pretrain(pretrain_model_path,self.device)
        ...
        self.lat_min = 40.50817
        self.lat_max = 40.90002
        self.lon_min = -74.24254
        self.lon_max = -73.67979
    ...
    def load_pretrain(self, path=None, device=torch.device("cpu")):
        if path is not None:
            self.AC_target.assignment_net.load_state_dict(torch.load(path, map_location=device, weights_only=True))
            self.AC_training.assignment_net.load_state_dict(torch.load(path, map_location=device, weights_only=True))
            self.AC_target.ViT.load_state_dict(torch.load("ViT_"+path, map_location=device, weights_only=True))
            self.AC_training.ViT.load_state_dict(torch.load("ViT_"+path, map_location=device, weights_only=True))
    ...
    def reset(self, capacity = 1, train=True):
        ...
        random_integers = np.random.randint(0, self.EMS.shape[0], size=(self.num))
        self.observe_space[:, 0:2] = self.EMS[random_integers]
        self.observe_space[:, 6:8] = self.EMS[random_integers]
    ...
    def observe(self, order, current_time, exploration_rate=0):
        minute = order['minute']
        call_type = order['INITIAL_SEVERITY_LEVEL_CODE']

        plat, plon = np.array(order['lat']), np.array(order['lng'])
        p = np.vstack((plat, plon)).T
        distances, indices = self.hospital_tree.query(p)
        d = self.hospital[indices]
        minute = np.array(minute).reshape(-1, 1)

        minute = np.array(minute).reshape(-1, 1)
        call_type = np.array(call_type).reshape(-1, 1)

        order = np.concatenate([p, d, minute, call_type], axis=-1)
        ...
        p_matrix, _ = self.AC_training.act(img, x1, x2, x3, torch.from_numpy(self.current_order_num).to(self.device))
    ...
    def train(self, batch_size=8, train_times=1, show_pbar=False, train_actor=False, train_critic=True):
        ...
        for _ in pbar:
            loss = None
            img, worker_state, order_state, order_num, pool_order, action, reward, img_next, worker_state_next, order_state_next, order_num_next, pool_order_next, action_next = self.buffer.sample(batch_size)
            for i in range(len(worker_state)):
                img_temp, worker_state_temp, order_state_temp, order_num_temp, pool_order_temp, action_temp, reward_temp, img_next_temp, worker_state_next_temp, order_state_next_temp, order_num_next_temp, pool_order_next_temp, action_next_temp = img[i], worker_state[i], order_state[i], order_num[i], pool_order[i], action[i], reward[i], img_next[i], worker_state_next[i], order_state_next[i], order_num_next[i], pool_order_next[i], action_next[i]
                ...
                if train_critic:
                    if action_next_temp is not None:
                        ...
                        img_temp, worker_state_temp, order_state_temp, order_num_temp, pool_order_temp, action_temp, img_next_temp, worker_state_next_temp, order_state_next_temp, order_num_next_temp, pool_order_next_temp, action_next_temp = img_temp.to(
                            self.device), worker_state_temp.to(self.device), order_state_temp.to(
                            self.device), order_num_temp.to(self.device), pool_order_temp.to(
                            self.device), action_temp.to(self.device), img_next_temp.to(
                            self.device), worker_state_next_temp.to(self.device), order_state_next_temp.to(
                            self.device), order_num_next_temp.to(self.device), pool_order_next_temp.to(
                            self.device), action_next_temp.to(self.device)
                        x1, x2, x3 = norm(pool_order_temp, worker_state_temp, order_state_temp)
                        img_temp = img_norm(img_temp)
                        _, _, q_value, _ = self.AC_training(img_temp, x1, x2, x3, action_temp, order_num_temp)

                        x1_next, x2_next, x3_next = norm(pool_order_next_temp, worker_state_next_temp,
                                                         order_state_next_temp)
                        img_next_temp = img_norm(img_next_temp)
                        p_matrix2, x_emb2 = self.AC_target.act(img_next_temp, x1_next, x2_next, x3_next,
                                                               order_num_next_temp)

                        # # threshold = np.log(1 / len(pool_order_next_temp))
                        ...
                        if self.restrict:
                            ...
                            dis = np.sqrt(
                                np.sum((worker_pos[:, np.newaxis, :] - order_pos[np.newaxis, :, :]) ** 2, axis=-1))
                        ...
                        q_value_next = torch.min(q_value_next2[0], q_value_next2[1]).detach()
                        target = self.gamma * q_value_next + reward_temp

                        loss_critic = (target - q_value[0])**2 + (target - q_value[1])**2

                    else:
                        img_temp, worker_state_temp, order_state_temp, order_num_temp, pool_order_temp, action_temp= img_temp.to(self.device), worker_state_temp.to(self.device), order_state_temp.to(self.device), order_num_temp.to(self.device), pool_order_temp.to(self.device), action_temp.to(self.device)
                        x1, x2, x3 = norm(pool_order_temp, worker_state_temp, order_state_temp)
                        img_temp = img_norm(img_temp)
                        _, _, q_value, _ = self.AC_training(img_temp, x1, x2, x3, action_temp, order_num_temp)
                        target = reward_temp
                ...
                if train_actor:
                    img_temp, worker_state_temp, order_state_temp, order_num_temp, pool_order_temp = img_temp.to(self.device),worker_state_temp.to(self.device), order_state_temp.to(self.device), order_num_temp.to(self.device), pool_order_temp.to(self.device)
                    x1, x2, x3 = norm(pool_order_temp, worker_state_temp, order_state_temp)
                    img_temp = img_norm(img_temp)
                    p_matrix, x_emb, p_ori = self.AC_training.act(img_temp, x1, x2, x3, order_num_temp, True)
    ...
    def update(self, feedback_table, new_route_table ,new_route_time_table ,new_remaining_time_table ,new_total_travel_time_table, new_detour_table, reward, assignment_table, assignment_state, final_step=False, episode=1):
        ...
        results = Parallel(n_jobs=self.njobs)(
            delayed(single_update)(self.observe_space[i], self.current_orders[i], self.current_order_num[i], self.travel_route[i], self.travel_time[i], feedback_table[i], new_route_table[i], new_route_time_table[i], new_remaining_time_table[i], new_total_travel_time_table[i], new_detour_table[i], self.go_back)
...
def single_update(observe_space, current_orders, current_orders_num, current_travel_route, current_travel_time, feedback, new_route ,new_route_time, new_remaining_time, new_total_travel_time, new_detour_table, go_back):
    ...
    if step > 0 and current_orders_num != 0:
        ...
        if drop_num > 0:
            ...
            if not go_back:
                observe_space[4] = 0
            # go back
            current_travel_route, current_travel_time, back_time, _ = TSP_route((observe_space[0], observe_space[1]), [(observe_space[6], observe_space[7])])
            observe_space[3] = back_time[0]

...
def get_args():
    ...
    parser.add_argument("--pretrain_model_path",type=str,default="pretrain.pth")
...
def main():
    ...
    worker = Worker(buffer, lr=args.lr, gamma=args.gamma, max_step=args.max_step, num=args.worker_num, device=device, zone_table_path = args.zone_table_path, hospital_table_path = args.hospital_path, EMS_table = args.ems_path, model_path = args.model_path, njobs = args.njobs, bi_direction = args.bi_direction, dropout = args.dropout, compression = compression, pretrain_model_path=args.pretrain_model_path, rand_init = args.rand_init, restrict = args.restrict, noise_type = args.noise_type)
    ...
    while True:
        ...
        log = f"Epoch {j} | Reward: {reward} , Loss: {loss} , Served Order: {Pickup_Num} , Delivery Time: {Delivery} , Finished Order: {Finish_Num} , Pickup Time: {Pickup} , Confirmation Time: {Confirmation}"
        print(log)
